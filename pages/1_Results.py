...
if st.session_state.get("predictions") is None:
    ...
    show_overlay_histograms_column = st.checkbox(
        f"Enable Overlay Histograms for Peptide Lengths"
    )
    ...
    # Initialize the col1 and col2 variables for optimal spacing
    ...
    for i, column in enumerate(selected_columns):
        if column not in [
            ...
            fig_histogram = px.histogram(
                ...
                barmode="overlay"
                if show_overlay_histograms_column and unique_lengths > 1
                else "relative",
                text_auto=True,
            )
    ...
    # Add the histograms for "n_flank", "c_flank", and "best_allele" at the end, these dont generate a boxplot as they are not numeric
    order = ["L", "R", "L"]
    x = 0
    ...
    # Add legend in cursive at the bottom (st.caption to decrease font size and use greyish font color)
    st.subheader("Legend")
    st.caption(
